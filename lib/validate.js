...
function validate( opts, options ) {
	if ( !isObject( options ) ) {
		return new TypeError( format( '1R52V,FD', options ) );
	}
	...
	if ( hasOwnProp( options, 'series' ) ) {
		...
		if ( !isBoolean( opts.series ) ) {
			return new TypeError( format( '1R52o,GE', 'series', opts.series ) );
		}
	}
	if ( hasOwnProp( options, 'limit' ) ) {
		opts.limit = options.limit;
		if ( !isPositiveInteger( opts.limit ) ) {
			return new TypeError( format( '1R53P,Fv', 'limit', opts.limit ) );
		}
	}
	return null;
}
